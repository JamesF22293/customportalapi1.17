...
public class PortalPlacer {
    ...
    private static Optional<PortalUtil.Rectangle> createDestinationPortal(World world, BlockPos blockPos, BlockState frameBlock, Direction.Axis axis) {
        ...
        int i = world.getDimension().getHeight() - 1;
            ...
            do {
                do {
                    if (!var13.hasNext()) {
                        ...
                        int o;
                        if (d == -1.0D) {
                            blockPos2 = (new BlockPos(blockPos.getX(), MathHelper.clamp(blockPos.getY(), 70, world.getDimension().getHeight() - 10), blockPos.getZ())).toImmutable();
